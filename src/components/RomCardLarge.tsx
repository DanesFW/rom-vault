...
import { Check, ChevronDown, FolderOpen, ImageOff, Pencil, Play, Search, StickyNote, Trash2, X } from "lucide-react";
import ArtworkImage from "./ArtworkImage";
import ArtPickerModal from "./ArtPickerModal";
import { useArtwork } from "../hooks/useArtwork";
import type { RomEntry, BacklogStatus } from "../types";
import { BACKLOG_COLORS, formatLabel, regionPillStyle, DISC_PILL_STYLE, SIZE_PILL_STYLE, HASHEOUS_UNSUPPORTED_CONSOLES } from "../types";
...
function RomCardLargeInner({ rom, companyColor, cardImageMode, isSelected, onToggleSelect, onCycleBacklog, onSaveNote, onRenameRom, onDelete, onPlay, isControllerSelected = false, controllerOpen, onControllerClose, rowIndex }: Props) {
  ...
  useEffect(() => {
    ...
    setHasheousResult(
      rom.hasheous_title
        ...
        : rom.sha1 && !rom.hasheous_title ? "not-found"
        : undefined as any
    );
    setHashing(false);
  ...
  function handleDelete() {
    if (confirmDelete) { onDelete(rom.id); setShowDetail(false); }
    ...
    <>
      ...
      >
        ...
        {onToggleSelect && showCheckbox && (
          ...
          >
            {isSelected && <span style={{ color: "white", fontSize: 12, lineHeight: 1, fontWeight: 700 }}>✓</span>}
          </div>
        )}

        <div onClick={open} style={{
          cursor: "pointer", minHeight: 0,
          height: cardImageMode === "uniform-crop" ? 200 : undefined,
        }}>
          <ArtworkImage
        ...
        {/* Footer */}
        <div style={{ padding: "10px 10px 8px", minHeight: 110, display: "flex", flexDirection: "column", gap: 6 }}>
          <div style={{
          ...
          <div style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center", marginTop: "auto" }}>
            <span style={{
              ...
              background: `${companyColor}18`, color: companyColor, border: `1px solid ${companyColor}33`,
            }}>
              {formatLabel(rom.format, rom.filename)}
            </span>
            {rom.region && (
              <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 8px", borderRadius: 3, ...regionPillStyle(rom.region) }}>
                {rom.region}
              </span>
            )}
            {rom.file_size > 0 && <span style={{ fontSize: 11, fontWeight: 600, padding: "2px 7px", borderRadius: 3, ...SIZE_PILL_STYLE }}>{formatBytes(rom.file_size)}</span>}
            {Array.isArray(rom.discs) && rom.discs.length > 0 && (
              <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 3, ...DISC_PILL_STYLE }}>
          ...
          <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
            <button onClick={() => onCycleBacklog(rom)} style={{
              ...
              background: status ? `${statusColor}18` : "var(--bg-surface)",
              border: `1px solid ${statusColor}`,
              color: status ? statusColor : "var(--text-muted)",
            ...
            <button onClick={() => { setShowNote(p => !p); open(); }} style={{
              width: 26, height: 26, borderRadius: 4, flexShrink: 0,
              background: (rom.note || showNote) ? "#fbbf2422" : "var(--bg-surface)",
              border: `1px solid ${(rom.note || showNote) ? "#fbbf24" : "var(--border-lit)"}`,
              color: (rom.note || showNote) ? "#fbbf24" : "var(--text-muted)",
        ...
        >
          <div ref={detailRef} style={{
            ...
            <div style={{ flex: 1, minWidth: 0, position: "relative", display: "flex", flexDirection: "column", background: "#0d0d1e" }}>
              ...
              {!hasArt && <div style={{ position: "absolute", inset: 0, zIndex: 1, background: `linear-gradient(135deg, ${companyColor}18 0%, #0d0d22 60%)` }} />}
              ...
              <div style={{ position: "relative", zIndex: 5, overflowY: "auto", flex: 1, padding: "10px 13px 13px", display: "flex", flexDirection: "column", gap: 7 }}>
                ...
                <button onClick={handleDelete} style={{
                  ...
                  border: `1px solid ${confirmDelete ? "#f87171" : "rgba(255,255,255,0.15)"}`,
                  color: confirmDelete ? "#f87171" : "rgba(255,255,255,0.7)",
                  fontFamily: "var(--font)", fontSize: 11, fontWeight: 700,
                  padding: "6px 10px", borderRadius: 5, cursor: "pointer",
                }}>
                  <Trash2 size={11} />{confirmDelete ? "CONFIRM DELETE" : "REMOVE"}
                </button>
...
export const RomCardLarge = memo(RomCardLargeInner);
export default RomCardLarge;
